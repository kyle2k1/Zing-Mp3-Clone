...
const Sidebar: React.FC<SidebarProps> = ({ currentUser, children }) => {
  ...
  return (
    <section className={cn('flex overflow-hidden', 'h-screen')}>
      ...
      <div
        className={cn(
          'overflow-hidden sm:flex lg:w-54 flex flex-col justify-between bg-sidebarBackground h-screen',
      ...
      >
        ...
        <div
          className={cn(
            'flex flex-col h-[54px] items-center justify-evenly ',
            show ? 'w-sidebarHeight' : 'w-sidebarWidth',
            showPlayer ? 'mb-20' : 'mb-0',
          )}
        >
          <div className={cn('hidden h-px bg-sidebarActive sm:block w-full')} />
          <Box
            data={dataPlaylists}
            item={item}
          />
